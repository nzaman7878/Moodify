const MoodFace = ({ mood = "neutral", small = false }) => {
  ...
  const containerClass = small
    ? "h-5 w-5 shadow-sm"
    : "h-32 w-32 shadow-[0_0_40px_rgba(0,0,0,0.25)]";

  const eyeClass = `absolute rounded-full bg-slate-900 ${small ? "h-1 w-1" : "h-3.5 w-3.5"}`;
  const eyeLeft = small ? "left-1.5 top-1.5" : "left-8 top-11";
  const eyeRight = small ? "right-1.5 top-1.5" : "right-8 top-11";
    ...
    happy: {
      ...
      mouth: `absolute rounded-b-full border-slate-900 border-b-solid ${
        small
          ...
          : "bottom-6 left-1/2 -translate-x-1/2 h-9 w-14 border-b-4"
      }`,
    },
    sad: {
      bg: "bg-blue-400",
      ...
      extra: currentMood === "sad" && !small && (
        <div className="absolute right-7 top-14 h-4 w-2.5 rounded-b-full rounded-t-full bg-cyan-200 opacity-80" />
      ),
    },
    angry: {
      bg: "bg-red-500",
      ...
      extra: currentMood === "angry" && (
        <>
          <div className={`absolute rounded-full bg-slate-900 rotate-12 ${small ? "left-1 top-1 h-[1.5px] w-1.5" : "left-6 top-8 h-1.5 w-7"}`} />
          <div className={`absolute rounded-full bg-slate-900 -rotate-12 ${small ? "right-1 top-1 h-[1.5px] w-1.5" : "right-6 top-8 h-1.5 w-7"}`} />
        </>
      ),
    },
    surprised: {
      ...
      mouth: `absolute rounded-full border-slate-900 bg-transparent ${
        small
          ...
          : "bottom-6 left-1/2 -translate-x-1/2 h-7 w-6 border-4"
      }`,
    },
    neutral: {
      bg: "bg-yellow-300",
      glow: "shadow-[0_0_40px_rgba(253,224,71,0.35)]",
      mouth: `absolute rounded-full bg-slate-900 ${
        small
  ...
  return (
    <div
      className={`relative shrink-0 rounded-full transition-all duration-500 ${config.bg} ${containerClass} ${!small ? config.glow : ""}`}
    >
      {config.extra}
